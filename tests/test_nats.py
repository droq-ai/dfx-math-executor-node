...
async def test_api_import():
    ...
    try:
        ...
        assert app.title == "Droq Math Executor Node"
    except ImportError as e:
        pytest.skip(f"Could not import FastAPI app: {e}")


@pytest.mark.asyncio
async def test_dfx_nats_client_can_be_instantiated():
    """Test that we can create a dfx NATS client."""
    try:
        from dfx.nats import NATSClient
        ...
        assert client.nats_url == "nats://localhost:4222"
        assert client.stream_name == "test-stream"
        assert client.nc is None  # Not connected yet
        assert client.js is None  # Not connected yet
    except ImportError:
        pytest.skip("nats-py not installed")
